...
def _resolve_tool_blocks(round_response: str, native_tool_calls: list, round_num: int, is_api_model: bool = False):
    ...
    if not used_native:
        ...
        # Gate ONLY that fenced pattern for native models, not the whole parser:
        # explicit [TOOL_CALL]/<invoke>/<tool_code>/DSML markup leaked into content
        # is a real call the model couldn't emit structurally. Non-native /
        # textual-only models keep every pattern, fenced blocks included, since
        # that's their only tool channel.
...
async def stream_agent_loop(
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        # an illustrative fence that wasn't executed (native model, no real
        # native_tool_calls) must not be stripped from the persisted text either —
